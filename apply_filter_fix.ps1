# Script tu dong ghi de filter_restaurants.py voi ban fix moi nhat
# Chay script nay ngay trong thu muc goc cua project (noi co thu muc data_pipeline)
...
$targetPath = "data_pipeline\filter_restaurants.py"

if (-not (Test-Path $targetPath)) {
    Write-Host "LOI: Khong tim thay $targetPath. Hay chay script nay tu thu muc goc cua project." -ForegroundColor Red
    exit 1
}

Set-Content -Path $targetPath -Value $content -Encoding UTF8 -NoNewline
Write-Host "Da ghi de thanh cong: $targetPath" -ForegroundColor Green

$check = Select-String -Path $targetPath -Pattern "_flatten_field"
if ($check) {
    Write-Host "XAC NHAN: File da co ban fix (_flatten_field tim thay)." -ForegroundColor Green
} else {
    Write-Host "CANH BAO: Van khong tim thay _flatten_field sau khi ghi. Co van de khac." -ForegroundColor Red
}
